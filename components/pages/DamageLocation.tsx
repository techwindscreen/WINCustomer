...
import { Car, Info, Upload, X, RefreshCw } from 'lucide-react'
import Link from 'next/link'
import { useRouter } from 'next/router'
import Image from 'next/image'
import { generateArgicCode, GlassProperties } from '../utilities/argic-code-generator'
...
const damageDescriptions: DamageDescription = {
  ...
  'Quarter glass': {
    'Smashed': 'Quarter glass is completely broken or shattered requiring full replacement. The glass may have multiple cracks, holes, or be completely missing from the frame.',
    'Leaking': 'Water is entering the vehicle around the quarter glass seal or frame. This indicates compromised weatherproofing that may affect interior components and cause water damage.',
    'Faulty Mechanism': 'Structural issues with the quarter glass mounting, frame integrity, or seal failure. May include loose fitting, improper installation, or deteriorated mounting points affecting the glass stability.'
  },
  'Rear Window': {
...
const generateAndUploadArgicCode = async (quoteId: string) => {
    ...
    return (
      <div className="mt-8 border-b border-gray-200 pb-8">
        ...
        <div className="flex items-center mb-4 text-lg relative">
          ...
          {activeTooltip === title && (
            <div ref={tooltipRef} className={getTooltipPosition(windowWidth)}>
  ...
  const handleRemoveImage = async (imageUrl: string, index: number) => {
    try {
      // Remove image from the uploadedImages state
      ...
      if (quoteID) {
        try {
          // Get current images from database and remove the specific one
          const updatedImages = uploadedImages.filter((url, i) => i !== index);
          ...
          const saveResponse = await fetch('/api/save-images', {
            ...
            body: JSON.stringify({
              ...
              action: 'replace' // Add this to indicate we're replacing the entire array
            }),
          });

          if (!saveResponse.ok) {
            console.error('Failed to update images in database');
          } else {
            console.log('Images updated in database successfully');
          }
        } catch (saveError) {
          console.error('Error updating images in database:', saveError);
        }
      }
      
      // If no images left, reset to pending status
      if (uploadedImages.length === 1) { // Will be 0 after removal
        setGlobalImageUploadStatus('pending');
      }
  ...
  // Add useEffect to handle click outside tooltips
  useEffect(() => {
    ...
    // Only add event listener if a tooltip is active
    if (activeTooltip) {
                              ...
                              <>
                                {globalImageUploadStatus === 'uploaded' && uploadedImages.length > 0 ? (
                                  ...
                                  <div className="flex flex-col items-start">
                                    <span className="text-yellow-600 font-semibold mb-2">No images uploaded</span>
                                    <button
                                      onClick={() => setGlobalImageUploadStatus('pending')}
